...
typedef enum
{
	...
	SMART_PRESENCE_PRESET_CONFIG_LOW_POWER_WAKEUP_FOR_RITSN,
...
#define DEFAULT_PRESET_CONFIG SMART_PRESENCE_PRESET_CONFIG_LOW_POWER_WAKEUP_FOR_RITSN
...
typedef struct
{
	...
	uint8_t nominal_high_count;
	uint8_t nominal_low_count;
	bool    nominal_high_confirmed;
	bool    door_open_event;
} app_context_t;
...
static void set_config(acc_ref_app_smart_presence_config_t *app_config, smart_presence_preset_config_t preset)
{
	...
	switch (preset)
	{
		...
		case SMART_PRESENCE_PRESET_CONFIG_LOW_POWER_WAKEUP_FOR_RITSN:
			app_config->show_all_detected_zones = false;
			app_config->use_wakeup_mode         = true;

			// Wakeup config
		        // Wakeup zone: 69–70 cm
			app_config->wakeup_config.num_zones            = 1;
			app_config->wakeup_config.num_zones_for_wakeup = 1;
			acc_detector_presence_config_start_set(app_config->wakeup_config.presence_config, 0.69f);
			acc_detector_presence_config_end_set(app_config->wakeup_config.presence_config, 0.70f);
			...
			acc_detector_presence_config_sweeps_per_frame_set(app_config->wakeup_config.presence_config, 11);
			acc_detector_presence_config_automatic_subsweeps_set(app_config->wakeup_config.presence_config, false);
			acc_detector_presence_config_frame_rate_set(app_config->wakeup_config.presence_config, 3.0f);
			acc_detector_presence_config_frame_rate_app_driven_set(app_config->wakeup_config.presence_config, false);
			acc_detector_presence_config_reset_filters_on_prepare_set(app_config->wakeup_config.presence_config, true);
			acc_detector_presence_config_intra_detection_set(app_config->wakeup_config.presence_config, true);
			acc_detector_presence_config_intra_detection_threshold_set(app_config->wakeup_config.presence_config, 1.7f);
			acc_detector_presence_config_intra_frame_time_const_set(app_config->wakeup_config.presence_config, 0.15f);
			acc_detector_presence_config_intra_output_time_const_set(app_config->wakeup_config.presence_config, 0.30f);
			acc_detector_presence_config_inter_detection_set(app_config->wakeup_config.presence_config, true);
			acc_detector_presence_config_inter_detection_threshold_set(app_config->wakeup_config.presence_config, 0.8f);
			acc_detector_presence_config_inter_frame_deviation_time_const_set(app_config->wakeup_config.presence_config, 0.5f);
			acc_detector_presence_config_inter_frame_fast_cutoff_set(app_config->wakeup_config.presence_config, 1.0f);
			acc_detector_presence_config_inter_frame_slow_cutoff_set(app_config->wakeup_config.presence_config, 0.20f);
			acc_detector_presence_config_inter_output_time_const_set(app_config->wakeup_config.presence_config, 0.5f);
			acc_detector_presence_config_inter_frame_presence_timeout_set(app_config->wakeup_config.presence_config, 3);

			// Nominal zone (button press): 3–15 cm
			app_config->nominal_config.num_zones = 1;
			acc_detector_presence_config_start_set(app_config->nominal_config.presence_config, 0.03f);
			acc_detector_presence_config_end_set(app_config->nominal_config.presence_config, 0.15f);
			...
			acc_detector_presence_config_sweeps_per_frame_set(app_config->nominal_config.presence_config, 16);
			acc_detector_presence_config_automatic_subsweeps_set(app_config->nominal_config.presence_config, false);
			acc_detector_presence_config_frame_rate_set(app_config->nominal_config.presence_config, 20.0f);
			acc_detector_presence_config_frame_rate_app_driven_set(app_config->nominal_config.presence_config, false);
			acc_detector_presence_config_reset_filters_on_prepare_set(app_config->nominal_config.presence_config, true);
			acc_detector_presence_config_intra_detection_set(app_config->nominal_config.presence_config, true);
			acc_detector_presence_config_intra_detection_threshold_set(app_config->nominal_config.presence_config, 1.3f);
			acc_detector_presence_config_intra_frame_time_const_set(app_config->nominal_config.presence_config, 0.15f);
			acc_detector_presence_config_intra_output_time_const_set(app_config->nominal_config.presence_config, 0.30f);
			acc_detector_presence_config_inter_detection_set(app_config->nominal_config.presence_config, true);
			acc_detector_presence_config_inter_detection_threshold_set(app_config->nominal_config.presence_config, 1.0f);
			acc_detector_presence_config_inter_frame_deviation_time_const_set(app_config->nominal_config.presence_config, 0.5f);
			acc_detector_presence_config_inter_frame_fast_cutoff_set(app_config->nominal_config.presence_config, 6.0f);
                                    ...
                                    smart_presence_mode_t          *new_mode)
{
	...
	/* This touchless flow does not use the reference app's switch delay. */
	result->switch_delay      = false;
	result->wakeup_detections = context->wakeup_detections;

	return status;
}

#define DOOR_DETECTION_COUNT 7U
...
{
    /* 호출마다 새 이벤트를 판정한다. */
    context->door_open_event = false;

    if (context->current_mode == SMART_PRESENCE_MODE_WAKEUP)
    {
        /* Wakeup zone은 1개: 감지되면 Nominal로 전환한다. */
        if (result->presence_detected &&
            result->zone_results[0].zone_detection)
        {
            context->nominal_high_count     = 0;
            context->nominal_low_count      = 0;
            context->nominal_high_confirmed = false;
    ...
    if (context->current_mode == SMART_PRESENCE_MODE_NOMINAL)
    {
        if (!context->nominal_high_confirmed)
        {
            /* 1단계: Nominal에서 연속 HIGH 7회 확인 */
            if (result->presence_detected)
            {
                if (++context->nominal_high_count >= DOOR_DETECTION_COUNT)
                {
                    context->nominal_high_confirmed = true;
                    context->nominal_low_count      = 0;
                }
            }
            else
        ...
        /* 2단계: HIGH 확인 후 연속 LOW 7회 확인 */
        if (!result->presence_detected)
        {
            if (++context->nominal_low_count >= DOOR_DETECTION_COUNT)
            {
                context->door_open_event = true;
